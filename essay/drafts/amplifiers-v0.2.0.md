...
**Version:** 0.2.0-draft (complete, polished)  
...
That second question turns out to be one of the hardest and most consequential in modern science. It arises wherever a system may be approaching a regime boundary: in climate dynamics, where the question is whether an ice sheet or an ocean circulation pattern is nearing a tipping point; in quantum devices, where operation near motional or phase-stability edges can dramatically enhance sensitivity; and in rare-event searches, where the challenge is to detect faint signatures near the limit of discrimination between signal and background, in a regime where sensitivity and ambiguity rise together. The mathematics needed to formalise transition sensing — bifurcation theory, critical slowing down, early warning signals — did not exist in the 19th century. It exists now. And it suggests that the sentinel tradition, long dismissed as pre-scientific, may have been asking the right question in the wrong century.
...
An instrument can be modelled as a mapping *f* from an environmental state (described by *n* variables) to an output (described by *m* variables). Operational invertibility requires, at minimum, a sufficiently informative output space (*m* ≥ *n*) and a locally nondegenerate response map over the relevant operating range. When these conditions hold, the output can in principle be mapped back to a unique environmental state. A mercury barometer satisfies this for the variable it is designed to measure: one input (pressure), one output (column height), and a monotonic relationship between them.
...
The superconducting quantum interference device (SQUID) is perhaps the most familiar example of near-critical sensing in precision physics. A SQUID operates at the boundary between superconducting and normal states, exploiting the macroscopic quantum coherence of the superconducting condensate. At the heart of the device is a Josephson junction — a weak link where the superconducting order parameter is suppressed — biased near its critical current. In this regime, a tiny change in magnetic flux through the loop produces a large change in the voltage across the junction. The gain is extraordinary: SQUIDs routinely detect magnetic fields at the femtotesla level.
...
[^10]: For the FitzHugh–Nagumo model, see R. FitzHugh, "Impulses and physiological states in theoretical models of nerve membrane," *Biophysical Journal* 1 (1961): 445–466, and J. Nagumo, S. Arimoto, and S. Yoshizawa, "An active pulse transmission line simulating nerve axon," *Proceedings of the IRE* 50 (1962): 2061–2070. For neuromorphic sensing near bifurcations, see the growing literature on event-driven and spiking sensor architectures.
